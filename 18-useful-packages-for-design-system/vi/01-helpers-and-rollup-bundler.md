...
> [!NOTE]
> Hãy hình dung một design system giống như một **căn bếp chuyên nghiệp**. **Style Dictionary** là kho nguyên liệu trung tâm — một nguồn chân lý duy nhất cho mọi nguyên liệu (màu sắc, spacing, font chữ). **clsx** là người đầu bếp tại quầy quyết định bày những loại trang trí nào lên đĩa dựa trên order. **CVA** là tấm thẻ công thức được chuẩn hóa đảm bảo mọi món "cay vừa" đều có hương vị giống hệt nhau. Và **Rollup** là dây chuyền đóng gói đóng hộp món ăn hoàn chỉnh để bất kỳ khách hàng (app) nào cũng có thể hâm nóng lại một cách gọn gàng. Mỗi công cụ giải quyết một nhiệm vụ, và cùng nhau chúng tạo ra một kết quả đáng tin cậy, lặp lại được.

> [!TIP]
> Bạn không cần phải áp dụng cả bốn công cụ cùng một lúc. Chúng độc lập với nhau. Nhưng trong một thư viện component trưởng thành, chúng củng cố lẫn nhau: tokens cung cấp dữ liệu cho CSS của bạn, `clsx`/`tailwind-merge` giải quyết xung đột class lúc runtime, CVA tổ chức các variant, và Rollup vận chuyển tất cả.
...
## 🎨 1. Style Dictionary — Nguồn chân lý duy nhất của bạn cho Tokens

Một **design token** là quyết định nhỏ nhất, có tên, trong một design system: một màu sắc, một bước spacing, một cỡ chữ, một độ đậm font. Thay vì rải rác `#7c3aed` khắp 40 file, bạn đặt tên cho nó một lần (`color.violet`) và tham chiếu đến cái tên đó ở mọi nơi.
...
- **Quản lý tập trung** — tokens nằm trong một nguồn chân lý duy nhất (JSON), dễ bảo trì và cập nhật.
...
- **Tự động hóa** — việc biến đổi tokens thành code có thể dùng được trở thành một câu lệnh build duy nhất.
...
Tạo một thư mục `tokens/`. Các file kết thúc bằng `.tokens.json` để cấu hình có thể glob chúng.
...
Tạo `config.js` ở thư mục gốc của dự án. Glob `source` lấy mọi file token, và mỗi mục dưới `platforms` mô tả một định dạng đầu ra.
...
> [!TIP]
> Muốn có cả đầu ra Android hoặc iOS? Thêm một khối platform khác với `transformGroup` phù hợp (ví dụ `android`). Một nguồn JSON duy nhất có thể cung cấp năng lượng cho web app, native app và tài liệu của bạn cùng một lúc — đó là sức mạnh thực sự của tokens.

---

## 🔗 2. clsx — Nối Class theo Điều kiện

`clsx` là một tiện ích siêu nhỏ để **nối các class name theo điều kiện**. Nó cho phép bạn thêm hoặc bớt các class CSS dựa trên một điều kiện mà không cần ghép chuỗi thủ công hay những mớ toán tử ba ngôi rối rắm.
...
> [!WARNING]
> `clsx` chỉ **nối** các chuỗi — nó hoàn toàn không biết các utility của Tailwind có ý nghĩa gì. Nếu bạn truyền vào `clsx('px-4', 'px-6')` bạn sẽ nhận được đúng nghĩa đen `"px-4 px-6"`, và thứ tự khai báo trong source CSS quyết định bên nào thắng. Để làm cho class *cuối cùng* thắng đối với các utility Tailwind xung đột, hãy kết hợp `clsx` với `tailwind-merge` (xem helper `cn` bên dưới).

### Helper `cn` chuẩn công nghiệp

Trong các thư viện component thực tế (được phổ biến bởi `shadcn/ui`), `clsx` được kết hợp với **`tailwind-merge`** để vừa nối *vừa* giải quyết xung đột các class Tailwind:
...
> [!TIP]
> Luôn bọc danh sách class của một component trong `cn(...)` và chuyển tiếp `className` của người dùng vào cuối cùng. Điều đó cho người dùng toàn quyền kiểm soát styling trong khi vẫn giữ các giá trị mặc định của bạn hợp lý.
...
Bạn gọi `cva(baseClasses, config)`. Đối số đầu tiên là phần base luôn được áp dụng; `config.variants` mô tả từng trục biến thiên; `defaultVariants` điền vào bất cứ thứ gì người dùng bỏ qua.
...
> [!TIP]
> Bởi vì các key variant đều có kiểu, editor của bạn sẽ tự động hoàn thành `color="primary" | "secondary"` và đánh dấu lỗi gõ nhầm ngay lúc compile. Kết hợp CVA với helper `cn` để người dùng vẫn có thể truyền thêm một `className` nhằm ghi đè đầu ra của variant.

---

## ⚡ 4. Rollup — Đóng gói Thư viện

Trong khi **Vite** và **Webpack** xuất sắc trong việc build *ứng dụng*, **Rollup** là tiêu chuẩn vàng để đóng gói các *thư viện* JavaScript — nó tạo ra đầu ra nhỏ, sạch, có thể tree-shake được ở nhiều định dạng module.
...
| **Tree Shaking** | Phân tích tĩnh cực kỳ hiệu quả | Tốt, nhưng tạo ra nhiều code bao quanh hơn |
| **Kích thước bundle** | Dung lượng tối giản, đầu ra sạch | Bao gồm các module loader và code runtime |
...
> [!WARNING]
> Việc không sử dụng `rollup-plugin-peer-deps-external` có thể khiến bundle của bạn chứa các bản sao trùng lặp của React. Khi một consumer import thư viện của bạn, React sẽ ném ra lỗi vi phạm "rules of hooks" do có nhiều instance React chạy đồng thời trong cùng một app.
...
### 1. Style Dictionary giải quyết vấn đề gì, và nguồn chân lý duy nhất của nó là gì?
<details>
  <summary><b>Reveal Answer</b></summary>

  Style Dictionary giải quyết vấn đề các quyết định thiết kế bị trùng lặp, không nhất quán rải rác khắp một codebase. Bạn định nghĩa **design tokens** (màu sắc, spacing, font chữ) một lần duy nhất trong các **file JSON** — nguồn chân lý duy nhất — và Style Dictionary biến đổi chúng thành nhiều định dạng đầu ra (CSS variables, SCSS variables, JS objects, các định dạng native) thông qua một câu lệnh build duy nhất. Cập nhật token một lần, build lại, và mọi nền tảng đều đồng bộ.
</details>
...
<details>
  ...
  Khai báo React dưới dạng `dependencies` buộc npm cài đặt một instance React riêng biệt bên trong `node_modules` của thư viện. Điều này dẫn đến hai instance React hoạt động đồng thời trong app của người dùng, làm hỏng rules of hooks. Bằng cách đặt React trong `peerDependencies`, bạn yêu cầu ứng dụng host cung cấp React, tránh được việc không khớp phiên bản và các lỗi instance trùng lặp.
</details>
...
<details>
  ...
  - **CJS (CommonJS)** sử dụng `require()`/`module.exports`. Nó là động (dynamic) và được tiêu thụ bởi Node.js cùng các build system cũ.

  Việc ship cả hai (cùng với một `.d.ts` được gộp lại qua `rollup-plugin-dts`) tối đa hóa khả năng tương thích: các bundler hiện đại chọn bản ESM có thể tree-shake, trong khi công cụ cũ lùi về CJS.
</details>
...
5. **Nâng cao:** thêm một file `tokens/spacing.tokens.json`, build lại, và xác minh rằng các spacing variable xuất hiện trong cả ba đầu ra mà không cần đụng đến `config.js`.
...
5. Xây dựng một component `<Button>` có `className` là `cn(buttonStyles({ color, size, state }), className)` để người dùng vẫn có thể ghi đè styles.
...
### 🛠️ Bài tập 3: Phòng thí nghiệm Biên dịch với Rollup
