...
Ở các phần trước, bạn đã học cách *sử dụng* các thư viện component có sẵn như DaisyUI và Shadcn/ui. Bây giờ chúng ta đảo ngược góc nhìn và trở thành **người tạo ra** một design system. Các công ty lớn không kéo các utility class rời rạc vào từng repository — họ phát hành một **nguồn dữ liệu gốc duy nhất** (single source of truth) được đánh phiên bản cho màu sắc, khoảng cách, bo góc, typography và component, và mọi đội sản phẩm đều sử dụng nó theo đúng một cách giống hệt nhau.

Trong bài học này, chúng ta sẽ xây dựng hệ thống đó từ đầu đến cuối dưới dạng một **monorepo** sử dụng **Yarn workspaces**. Chúng ta tạo ba package: `foundation` (design tokens), `reactjs` (các component sử dụng tokens), và `storybook` (tài liệu sống). Thứ gắn kết tất cả lại với nhau là **Style Dictionary**, một công cụ biến đổi token JSON độc lập nền tảng thành CSS, SCSS, JavaScript thực sự, và một **Tailwind preset** mang thương hiệu mà mọi component sau đó đều dùng.

---

## ⚡ 1. Khái Niệm & Tổng Quan: Một Nguồn Dữ Liệu Gốc Duy Nhất

Một design system trả lời một câu hỏi đơn giản nhưng tốn kém: *"Màu vàng cảnh báo (warning) chính xác của thương hiệu chúng ta là gì, và làm sao để cả 40 app của chúng ta đều dùng nó giống hệt nhau?"* Nếu mỗi đội tự hard-code `#FBBF24`, thì đến ngày thương hiệu thay đổi, bạn sẽ phải lục tìm khắp hàng chục repository. Thay vào đó, chúng ta định nghĩa giá trị **một lần** dưới dạng token, rồi sinh ra mọi thứ khác từ đó.
...
> [!TIP]
> Cách chia tách monorepo này là có chủ đích và được định hình theo quan hệ phụ thuộc. `foundation` không biết gì về React. `reactjs` phụ thuộc vào `foundation`. `storybook` phụ thuộc vào **cả hai**. Vạch đúng chiều mũi tên phụ thuộc này là quyết định kiến trúc quan trọng nhất trong toàn bộ phần này — các tầng thấp hơn không bao giờ được phép import lên trên.
...
JSON thô không bao giờ được gửi tới trình duyệt. **Style Dictionary** là cỗ máy đọc JSON một lần và dập ra một file CSS, một file SCSS, một JS object, và một Tailwind preset. Các component React được xây dựng dựa trên Tailwind preset đó (nhờ vậy lập trình viên chỉ cần viết `company-bg-warning-500`), còn Storybook là phòng trưng bày nơi mọi component hoàn thiện được hiển thị.

| Package | Alias | Phụ thuộc vào | Trách nhiệm |
| :--- | :--- | :--- | :--- |
| `foundation` | `@company-ds/foundation` | — | Token JSON + cấu hình Style Dictionary + output được sinh ra + Tailwind preset |
| `reactjs` | `@company-ds/reactjs` | `foundation` | Các component React được style bằng các class Tailwind mang thương hiệu |
| `storybook` | `@company-ds/storybook` (private) | `foundation` + `reactjs` | Tài liệu trực quan; import `output.css` đã được build |

---

## ⚡ 3. Dựng Khung Monorepo Với Yarn Workspaces 🛠️

Chúng ta bắt đầu từ một thư mục rỗng và chuyển sang dùng Yarn phiên bản hiện đại (Berry), rồi tạo bộ khung cho các package.
...
Bây giờ hãy báo cho `package.json` ở **root** biết rằng mọi thứ nằm dưới `packages/*` đều là một workspace, và đặt cho mỗi package một tên alias có scope.
...
> [!WARNING]
> Các tên có scope (`@company-ds/...`) không chỉ để cho đẹp — các lệnh `yarn workspace <name> add ...` bên dưới nhắm tới package **theo tên**, chứ không phải theo thư mục. Nếu bạn đổi tên, hãy sửa mọi lệnh cho khớp. Hãy chọn prefix thương hiệu thật của bạn ngay từ đầu và giữ nhất quán.

Thiết lập các quan hệ phụ thuộc bằng lệnh workspace. Yarn liên kết chúng bằng giao thức `workspace:` — không cài đặt qua mạng, chỉ tạo symlink bên trong repo.
...
Trong một monorepo, bạn viết các quy tắc compiler nghiêm ngặt **một lần** ở root và để mỗi package `extends` nó, chỉ ghi đè những gì riêng biệt (`baseUrl`, `outDir`, chế độ `jsx` của riêng nó).
...
> [!TIP]
> Ánh xạ `paths` với `"@company-ds/*": ["packages/*/lib"]` chính là thứ giúp `import Button from "@company-ds/reactjs"` phân giải được tới output đã biên dịch khi type-check. Thư mục `lib` là *sản phẩm build* — đừng bao giờ sửa nó bằng tay; nó sẽ được tạo lại sau mỗi lần build.

---

## ⚡ 5. Package `foundation`: Design Tokens Dưới Dạng JSON 🎨

Bên trong `packages/foundation/src/tokens/`, chúng ta viết một file JSON cho mỗi nhóm token. Những file này là nơi **duy nhất** mà một giá trị hex thô hay con số pixel được phép tồn tại trong toàn bộ hệ thống.
...
Style Dictionary đọc mọi file token `*.json` và phát ra một output cho mỗi **platform** mà ta cấu hình. Chúng ta sinh ra ba đích đến: biến SCSS, CSS custom properties, và một JS object.
...
File `tokens.css` được sinh ra sẽ chứa các mục như sau — bằng chứng cho thấy ý định đã trở thành code thực sự:
...
> [!WARNING]
> Mọi thứ nằm dưới `lib/` đều là **output được sinh ra**. Hãy thêm nó vào `.gitignore` (hoặc commit có chủ đích như một sản phẩm phát hành) nhưng **đừng bao giờ sửa bằng tay** — lần chạy `build:tokens` tiếp theo sẽ ghi đè lên thay đổi của bạn. Hãy sửa JSON trong `src/tokens/` rồi build lại.

---

## ⚡ 7. Sinh Ra Một Tailwind Preset Có Thương Hiệu 🧩

Đây chính là thành quả. Chúng ta đưa các JS token đã biên dịch vào một Tailwind **preset** và thêm một **prefix** (tên thương hiệu của bạn). Prefix là lý do lập trình viên viết `company-bg-warning-500` thay vì `bg-warning-500` thuần — nó đảm bảo các class của design system không bao giờ xung đột với cấu hình Tailwind của đội khác.
...
Cài React (và types của nó) dưới dạng **dev** dependency, nhưng khai báo React là **peer** dependency — chính app sử dụng sẽ cung cấp React, còn thư viện của bạn không được đóng gói kèm bản React riêng của mình.
...
Cấu hình Tailwind của package component chỉ đơn giản là áp dụng preset mang thương hiệu đã được build trong `foundation`:
...
Giờ thì một component được style hoàn toàn bằng các class token mang thương hiệu — không có hex thô, không có con số tùy tiện:
...
> [!NOTE]
> `peerDependencies` với `>=16.8.0` có nghĩa là "Tôi cần React 16.8 trở lên (kỷ nguyên hooks), nhưng chính *app sử dụng* mới sở hữu bản cài đặt thực sự." Điều này ngăn lỗi kinh điển khi có hai bản React cùng nằm trong một bundle, vốn làm hỏng hooks lúc chạy.

---

## ⚡ 9. Package `storybook`: Kết Nối Phòng Trưng Bày

Khởi tạo Storybook bên trong package, rồi cho file preview của nó import `output.css` đã **build** để phòng trưng bày render bằng đúng các style đã biên dịch mà người dùng cuối sẽ nhận được.
...
Bước kết nối Storybook quan trọng nhất chính là import file CSS đã biên dịch trong `preview.ts`.
...
Chỉ một lần sửa JSON là lan tỏa khắp toàn bộ tổ chức. Đó chính là toàn bộ giá trị cốt lõi của một design system cấp doanh nghiệp: **sửa một lần, lan tỏa mọi nơi, có bảo đảm.**

---

## 🧠 Test Your Knowledge

Trả lời các câu hỏi sau để kiểm tra mức độ hiểu của bạn. Nhấn **Reveal Answer** để kiểm chứng đáp án.

### 1. "Design token" chính xác là gì, và tại sao lưu nó dưới dạng JSON thay vì CSS?
<details>
  <summary><b>Reveal Answer</b></summary>

  Một design token là một quyết định thiết kế được đặt tên, **độc lập nền tảng**, lưu dưới dạng dữ liệu, ví dụ `color.warning.500 = "#F59E0B"`. Lưu nó dưới dạng JSON (thay vì trực tiếp là CSS) nghĩa là giá trị đó chỉ là *ý định* (intent), tách rời khỏi bất kỳ nền tảng đơn lẻ nào. Một công cụ như Style Dictionary sau đó có thể biên dịch chính mục JSON đó thành CSS custom properties, biến SCSS, một JS object, iOS Swift, Android XML, v.v. — giữ cho web, mobile và công cụ thiết kế luôn đồng bộ hoàn hảo từ một nguồn dữ liệu gốc duy nhất.
</details>
...
<details>
  ...
  Quan hệ phụ thuộc chỉ được phép chảy **xuống dưới**. `foundation` là tầng thấp nhất — token thuần túy, không biết gì về React hay việc render, nên nó không phụ thuộc vào cái gì cả. `reactjs` sử dụng các token/preset đã biên dịch, nên nó phụ thuộc vào `foundation`. `storybook` là phòng trưng bày dùng để tài liệu hóa các *component* (từ `reactjs`) được style bằng các *token* (từ `foundation`), nên nó phải phụ thuộc vào cả hai. Nếu một tầng thấp hơn lại import một tầng cao hơn, bạn sẽ tạo ra phụ thuộc vòng (circular dependency) và làm hỏng quá trình build.
</details>
...
<details>
  ...
  Một thư viện không nên đóng gói kèm bản React riêng của nó; chính **app sử dụng** mới sở hữu bản cài đặt React. Khai báo `react`/`react-dom` là `peerDependencies` có nghĩa là "Tôi yêu cầu React 16.8+ (kỷ nguyên hooks) nhưng bạn là người cung cấp nó." Điều này ngăn lỗi kinh điển khi có hai instance React khác nhau cùng nằm trong một bundle, vốn khiến hooks ném ra lỗi "Invalid hook call" lúc chạy. Trong quá trình phát triển, bạn vẫn cài React dưới dạng `devDependency` để package có thể biên dịch và chạy Storybook cục bộ.
</details>
...
2. Chạy `yarn workspace @company-ds/foundation build` và xác nhận giá trị này xuất hiện trong `lib/tokens/css/tokens.css` dưới dạng `--color-brand-500`.
3. Build lại các component bằng `yarn workspace @company-ds/reactjs build:css`.
4. Trong `button.tsx`, đổi class thành `company-bg-brand-500` và khởi chạy Storybook để xác nhận màu mới được hiển thị — mà không hề chạm vào bất kỳ giá trị hex nào trong component.

### 🧩 Bài Tập 2: Thêm Một Output Nền Tảng Thứ Hai (JSON Cho Một Đội Mobile)
1. Trong `style-dictionary.config.js`, thêm một platform thứ tư tên là `json` với `transformGroup: "js"`, `buildPath: "lib/tokens/json/"`, và một file `{ destination: "tokens.json", format: "json/flat" }`.
2. Chạy `yarn workspace @company-ds/foundation build:tokens`.
3. Kiểm tra file `lib/tokens/json/tokens.json` được sinh ra — quan sát cách *cùng một* bộ token nguồn giờ cũng có sẵn dưới dạng JSON phẳng mà một đội iOS/Android native có thể sử dụng.
4. Suy ngẫm: bạn đã thêm một nền tảng tiêu thụ hoàn toàn mới mà không sửa một giá trị token nào. Hãy ghi chú trong một comment lý do điều này chứng minh được lợi ích của "single source of truth".
