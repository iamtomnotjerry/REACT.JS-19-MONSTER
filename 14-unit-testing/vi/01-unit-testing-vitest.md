...
**Kiểm thử đơn vị (Unit Testing)** là việc kiểm thử các phần mã nguồn riêng lẻ (hàm, lớp tiện ích, thuật toán) một cách hoàn toàn độc lập nhằm đảm bảo chúng hoạt động chính xác đúng như mong đợi. Trong hệ sinh thái React hiện đại, **Vitest** là framework kiểm thử thế hệ mới được ưa chuộng vì cực kỳ nhanh, cấu hình tức thì với Vite, và hỗ trợ cú pháp assertion tương thích với Jest.
...
Bài học này tập trung hoàn toàn vào **kiểm thử đơn vị** — xác minh rằng những phần logic nhỏ, độc lập làm đúng những gì ta mong đợi. Hãy hình dung như một nhà máy sản xuất ô tô: trước khi lắp ráp toàn bộ chiếc xe (E2E), bạn kiểm tra để chắc chắn từng con bu-lông, bánh răng và bugi hoạt động đúng khi đứng riêng (unit). Nếu một con bu-lông bị lỗi, bạn muốn biết *ngay bây giờ*, chứ không phải sau khi xe đã xuất xưởng.
...
> [!WARNING]
> Test đơn vị không bao giờ nên phụ thuộc vào mạng, cơ sở dữ liệu hay hệ thống tệp thực. Ngay khi một test "đơn vị" gọi tới một dịch vụ bên ngoài, nó trở thành một test *tích hợp* chậm và hay lỗi vặt. Hãy giữ cho các đơn vị thuần khiết và độc lập — mock các phụ thuộc bên ngoài khi cần.
...
Thêm một script để khởi chạy trình chạy Vitest ở chế độ watch, cùng với một bảng điều khiển UI tùy chọn:
```json
"scripts": {
  "test": "vitest",
  "test:ui": "vitest --ui"
...
Script `test:ui` mở một bảng điều khiển trên trình duyệt, nơi bạn có thể lọc theo các test đậu, rớt, hoặc bị bỏ qua, và xem chi tiết mã cũng như đầu ra console của từng test. Script `test` thông thường chạy cùng bộ test đó trực tiếp trong terminal của bạn.
...
3. **`expect`**: Khẳng định rằng giá trị nhận được khớp với kết quả mong đợi. Một **assertion** là một câu lệnh kiểm tra xem một giá trị có thỏa mãn một điều kiện hay không (bằng nhau, tính truthy, v.v.).
...
> [!TIP]
> Luôn đặt cho các khối `it`/`test` của bạn những **mô tả có ý nghĩa** (ví dụ `'should throw an error when dividing by zero'`). Khi một test rớt sau sáu tháng — hoặc khi một đồng đội đọc bộ test của bạn — phần mô tả chính là manh mối đầu tiên cho biết thứ gì đã hỏng và tại sao.
...
**TDD** đảo ngược thứ tự thông thường: bạn viết **test trước**, rồi viết vừa đủ mã để làm cho nó đậu. Chu trình gồm ba bước lặp lại, thường gọi là **Red → Green → Refactor**:
...
| `toBeTruthy()` | Giá trị là truthy (bất cứ gì trừ `false`, `0`, `''`, `null`, `undefined`, `NaN`) | Kiểm tra nới lỏng kiểu "có giá trị" |
| `toBeFalsy()` | Giá trị là falsy (một trong các giá trị liệt kê ở trên) | Kiểm tra nới lỏng kiểu "rỗng/vắng mặt" |
...
> [!WARNING]
> `toThrow()` yêu cầu bạn truyền lời gọi **được bọc trong một hàm mũi tên**: `expect(() => myFn()).toThrow()`. Nếu bạn viết `expect(myFn()).toThrow()`, hàm sẽ chạy *ngay lập tức* và làm crash test trước khi assertion kịp bắt được lỗi.
...
Một bộ test vững chắc kiểm thử ba loại đầu vào, không chỉ luồng thuận lợi (happy path):
...
| **Tiêu cực (Negative)** | Hệ thống từ chối đầu vào **không hợp lệ** một cách an toàn (ném lỗi, không crash) | `add('3', 5)` ném ra "Both inputs must be numbers" |
| **Biên (Boundary)** | Hệ thống xử lý **các điểm biên** của khoảng hợp lệ, nơi lỗi ẩn nấp nhiều nhất | Một bộ kiểm tra mật khẩu tại chính xác 8 và chính xác 16 ký tự |
...
Các biên là nơi trú ngụ của lỗi lệch-một (off-by-one). Đối với một mật khẩu phải **từ 8 đến 16 ký tự**, bạn kiểm thử *ngay bên trong* và *ngay bên ngoài* mỗi điểm biên:
...
<details>
  ...
  **AAA** = **Arrange** (thiết lập đầu vào/dữ liệu mock), **Act** (gọi hàm đang được kiểm thử), **Assert** (xác minh kết quả). Nó giữ cho test dễ đọc, giống như tập hợp nguyên liệu, nướng bánh, rồi nếm thử.
  ...
  Đối với `toThrow()`, chúng ta bọc lời gọi trong một hàm mũi tên — `expect(() => myFn()).toThrow()` — để Vitest có thể gọi nó *bên trong một try/catch*. Nếu bạn truyền trực tiếp `expect(myFn())`, hàm sẽ thực thi ngay lập tức và lỗi làm crash test trước khi assertion kịp chạy.
</details>
...
<details>
  ...
  - **Kiểm thử tiêu cực (Negative testing)** xác nhận hệ thống *từ chối một cách an toàn* các đầu vào không hợp lệ — ném ra một lỗi rõ ràng thay vì crash hoặc trả về dữ liệu rác.
  - **Kiểm thử biên (Boundary testing)** nhắm vào *các điểm biên* của khoảng hợp lệ (ngay bên trong và ngay bên ngoài giá trị nhỏ nhất/lớn nhất), vì lỗi lệch-một thường tụ tập tại các biên.
</details>
...
3. Tạo các tệp `.test.js` tương ứng và viết một bộ `describe` cho mỗi tệp, xác minh:
   ...
   - Dùng **`toContain`** để khẳng định rằng một mảng kết quả có chứa một phần tử cụ thể.
...
2. Trong `src/cartUtils.test.js`, xác minh rằng:
   ...
   - `formatCurrency` thêm ký hiệu đô la ở đầu và định dạng với hai chữ số thập phân — khẳng định bằng **`toMatch(/^\$\d+\.\d{2}$/)`**.
