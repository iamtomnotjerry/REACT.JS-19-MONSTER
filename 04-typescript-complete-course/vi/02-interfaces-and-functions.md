...
Trong TypeScript, hai công cụ quan trọng nhất để mô tả *hình dạng* (shape) của dữ liệu là **interface** và **type alias**. Thoạt nhìn chúng gần như giống hệt nhau, nhưng mỗi loại lại có những sức mạnh riêng mà loại kia không có. Một khi đã hiểu về hình dạng đối tượng, bạn có thể áp dụng sự chặt chẽ tương tự cho các **hàm** của mình: định nghĩa kiểu cho tham số, đối số tùy chọn, giá trị mặc định, định dạng giá trị trả về, và thậm chí cả các callback mà bạn truyền đi.
...
> [!WARNING]
> Chỉ có **interface** mới hỗ trợ *declaration merging*. Nếu bạn khai báo cùng một tên **type alias** hai lần, TypeScript sẽ báo lỗi `Duplicate identifier`. Nắm được sự khác biệt này sẽ giúp bạn tiết kiệm hàng giờ debug trong bối rối.
...
### 📊 Interface vs. Type Alias trong nháy mắt
...
TypeScript sử dụng hệ thống kiểu **structural** (theo cấu trúc), chứ không phải *nominal* (theo tên) — khác với Java hay C#. Điều này có nghĩa là hai kiểu được coi là tương thích nếu *cấu trúc* của chúng khớp nhau — *tên* của kiểu là không liên quan.
...
Trong TypeScript, bạn có thể định nghĩa kiểu một cách nghiêm ngặt cho tham số hàm, đối số tùy chọn, giá trị mặc định, và định dạng giá trị trả về.
