...
A component library is a single, versioned package that holds your reusable UI. Instead of copy-pasting a `Button` into five different apps (and fixing the same bug five times), you publish it once and every app installs the same source of truth. Update the package, bump the version, and every consumer upgrades with one `npm install`.
...
> [!WARNING]
> The most common library-authoring bug is **bundling React**. If your `dist` includes its own React, the consuming app ends up with *two* React instances and hooks crash with "Invalid hook call." The fix is the `rollup-plugin-peer-deps-external` plugin plus a `peerDependencies` entry — both covered below.
...
Now initialize and configure TypeScript for a *distributable* (note the emphasis on emitting declarations):
...
> [!NOTE]
> `emitDeclarationOnly: true` is the key library setting. We let **Rollup** (via its TypeScript plugin) handle JavaScript compilation and let `tsc` produce only the `.d.ts` files. This avoids duplicate/competing compilation pipelines.

---

## ⚡ 3. CVA — Type-Safe Variant Management (deep dive)

**class-variance-authority (CVA)** solves a real pain: as a component grows, its `className` logic degenerates into a tangle of ternaries. CVA lets you declare your variants as a structured object and returns a function that maps props → class string.

```bash
npm i class-variance-authority tailwind-merge
```

Mentally, `cva()` is a lookup table:
...
Here is the Button's variant configuration. We store the CVA result in `buttonStyles`:
...
There is a subtle problem: what if a consumer passes their own `className="bg-red-500"` to override your default `bg-blue-500`? Naively concatenating gives `"bg-blue-500 ... bg-red-500"` — **both** classes are in the string, and which wins depends on CSS source order, not author intent. That is fragile.

`tailwind-merge` understands Tailwind's class groups: when it sees two classes from the same group (two `bg-*`, two `text-*`, two `px-*`), it keeps **only the last one**. The consumer's override reliably wins.
...
> [!TIP]
> Order matters inside `twMerge`. Put the library's generated classes first and the consumer's `className` last. `tailwind-merge` resolves conflicts left-to-right, so whatever appears later wins — exactly the "consumer overrides library default" behavior you want.
...
**Rollup** is the bundler that turns `src/` into shippable `dist/`. We install Rollup and a set of plugins, each with one job.
...
The config exports an **array of two builds**: the first emits both CJS and ESM JavaScript; the second bundles the type declarations.
...
The `package.json` is the contract that tells npm and consuming bundlers *where to find what*. These fields are what make your library actually importable after install.
...
You will get a `dist/` containing `cjs/index.js`, `esm/index.js`, and `index.d.ts` — production-ready output. To ship it publicly:
...
Storybook renders each component in isolation so you can eyeball every variant. Initialize it and pick the **Vite** builder when prompted.
...
Delete the demo stories Storybook generates and write your own. Crucially, import your Tailwind CSS in `.storybook/preview.ts` so classes render:
...
> [!TIP]
> The `tags: ["autodocs"]` line tells Storybook to auto-generate a Docs page that reads your TypeScript props and lists every prop, its type, and its default. It is free, always-accurate documentation for your library's consumers.
...
Now write tests for the Button. The key matcher is `toHaveClass`, which asserts the CVA mapping produced the right Tailwind classes:
...
All five tests should pass — confirming variants, sizes, the disabled state, and `tailwind-merge` composition all work. You now have a component that is *built*, *bundled*, *visually verified*, and *unit-tested*.
...
<details>
  ...
  `defaultVariants` specifies which variant value is applied when the corresponding prop is **omitted** by the consumer. In our Button, `defaultVariants: { variant: "primary", size: "md", disabled: false }` means `<Button>Hi</Button>` renders as a primary, medium, enabled button without the consumer passing anything. It centralizes defaults in one place instead of scattering `?? "primary"` fallbacks throughout the component, and it keeps the derived TypeScript prop types accurate.
</details>
...
2. From the library folder run `npm pack` — this produces a `.tgz` exactly as `npm publish` would upload it. List its contents and confirm only `dist` (no `src`) is included, honoring the `files` field.
