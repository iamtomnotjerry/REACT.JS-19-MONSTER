...
> [!NOTE]
> Một **motion value** là một bộ chứa phản ứng đặc biệt theo dõi trạng thái của một thuộc tính có thể tạo hiệu ứng (như `x`, `opacity`, hay `backgroundColor`). Khi bạn dùng thuộc tính `animate`, Framer Motion tạo các motion value này *cho bạn một cách tự động* — đó chính là lý do tại sao một `animate={{ x: 200 }}` trần trụi đã trượt mượt mà dù bạn chưa từng viết một transition nào. Các hook trong bài học này đơn giản là cho phép bạn tự tạo và điều khiển các giá trị đó.

> [!TIP]
> Hãy dùng các **thuộc tính khai báo** (`animate`, `exit`, `layout`) cho 90% các hiệu ứng giao diện — chúng đơn giản hơn và tự tối ưu hóa. Chỉ hạ xuống dùng **motion-value hook** khi hiệu ứng phải phản ứng *liên tục* với một đầu vào bên ngoài (cuộn, con trỏ, kéo), bởi những đầu vào đó thay đổi quá thường xuyên để có thể mô hình hóa thành state React rời rạc mà không bị giật (jank).

> [!WARNING]
> Một khi bạn gắn một thuộc tính vào motion value **của riêng bạn** thông qua thuộc tính `style`, Framer Motion ngừng quản lý nó cho bạn. Điều đó có nghĩa là khả năng làm mượt tự động biến mất — phần tử nhảy tức thì đến từng giá trị mới. Nếu bạn muốn lấy lại sự mượt mà, hãy bọc giá trị đó trong **`useSpring`** (được đề cập bên dưới). Việc trộn lẫn `animate={{ x }}` và `style={{ x }}` cho *cùng một* thuộc tính tại cùng một thời điểm là một xung đột và sẽ không hoạt động như mong đợi.
...
Tạo hiệu ứng cho bố cục (`width`, `height`, `top`, `left`) theo cách ngây thơ thì **chậm**. Các thuộc tính như `width` và `top` buộc trình duyệt phải tính toán lại hình học của trang (gọi là "layout" hay "reflow") trên *từng frame một*, gây giật rất nặng. Các thuộc tính transform (`scale`, `translate`) thì **không** — chúng chạy trên bộ tổng hợp (compositor) của GPU và không bao giờ chạm vào layout.
...
> [!NOTE]
> **Hiệu năng cảm nhận (perceived performance)** mới là phần thưởng thực sự. Ngay cả khi tổng khối lượng công việc là tương đương, một hiệu ứng chạy mượt ở 60fps *cảm giác* nhanh hơn và đáng tin cậy hơn một bước nhảy tức thì, bởi mắt có thể theo dõi sự liền mạch và không bao giờ mất dấu một phần tử. Hiệu ứng layout gần như không tốn gì (chỉ là các transform thuần) nhưng làm cho giao diện cảm thấy chỉn chu và phản hồi tốt — một lợi tức khổng lồ cho chỉ một thuộc tính `layout`.
...
`useTransform` nhận một motion value nguồn và ánh xạ một **khoảng đầu vào** sang một **khoảng đầu ra**. Đầu ra có thể là số, màu sắc, hay đơn vị — khiến nó hoàn hảo để suy ra một thuộc tính hiệu ứng từ một thuộc tính khác. Ở đây chúng ta biến khoảng cách kéo ngang thành một sự thay đổi màu sắc:
...
Giá trị thô từ `useMotionValue` hoặc `useScroll` nhảy phắt đến mục tiêu của nó mà không có easing, có thể cảm thấy máy móc. **`useSpring`** bọc một motion value (hoặc một số thuần) và khiến nó *đuổi theo* mục tiêu của mình bằng một mô hình vật lý lò xo (spring) — mang lại chuyển động mượt mà, hơi nảy nhẹ. Thường thì bạn chỉ thay đổi một từ:
...
<details>
  ...
  Trong React thông thường, khi một state thay đổi để ẩn một component, React lập tức gỡ bỏ phần tử khỏi DOM. Không có thời gian để thực hiện các frame chuyển tiếp.
  `AnimatePresence` giải quyết điều này bằng cách can thiệp vào hành động unmount. Nó trì hoãn việc xóa node DOM cho đến khi transition `exit` của motion con lồng bên trong chạy xong hoàn toàn.
</details>
...
<details>
  ...
  `useScroll()` trả về các motion value mô tả vị trí cuộn, hữu ích nhất là `scrollYProgress` (một giá trị từ `0` ở đầu trang đến `1` ở cuối trang). Bạn thường đưa nó vào `useTransform` để điều khiển một thuộc tính như `scale` hay `opacity`. Vì giá trị thô bám theo cuộn một cách chính xác và có thể cảm thấy máy móc, bạn bọc nó trong `useSpring(scrollYProgress, { stiffness, damping })` để thêm quán tính dựa trên vật lý, tạo ra chuyển động mượt mà, tự nhiên.
</details>
...
6. **Thêm chiều sâu:** Bọc giá trị với `const smooth = useSpring(scrollYProgress, { stiffness: 100, damping: 30 });` và gắn thanh vào `smooth` thay thế. Cuộn nhanh và quan sát cách thanh nhẹ nhàng dịch vào vị trí với quán tính êm ái thay vì bám theo cuộn tức thì.
